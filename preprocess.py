...
def load_origin_data(data_dir, app_num):
    ...
    filenames = [filename for filename in os.listdir(data_dir) if os.path.isfile(os.path.join(data_dir, filename)) and filename.split(".")[1] == "num"]
...
def split_train_and_dev(datas, ratio=0.8, keep_ratio=1):
    ...
    for app_data in tqdm.tqdm(datas, ascii=True, desc='[Split]'):
        ...
        # config.keep_ratio参数用来进行小样本分析，取原始数据集config.keep_ratio的数据进行分析
